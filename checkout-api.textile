...
| decline_callback | When set to 0 does not redirect on decline ||
...
<pre><code><input type=checkbox name=items[] value="Logo T-shirt (sm): $15">
...
You could alternatively use multiple @name@ and @amount@ pairs.
For each @amount[]@ there must be a matching @name[]@.
